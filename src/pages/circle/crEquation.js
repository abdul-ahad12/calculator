...
const CrEquation = () => {
  ...
  const resultDumm = () => {
    ...
    let final = (
      <MathComponent tex={`Center:(${a},${b}) \\hspace{0.5cm} Radius:${c}`} />
    );
  ...
  const s =
    "\\displaylines{Centre: (-g,-f) \\\\ \\\\ Radius: \\sqrt{g^2+f^2-c}}";
  const f2 = <MathComponent tex={s} />;

  return (
    <div>
      <Helmet>
        <title>Center and radius from circle equation.</title>
        <meta
          name="description"
          content="Free calculator to find Center and radius from circle equation."
        />
      </Helmet>
      <TitleTemplate
